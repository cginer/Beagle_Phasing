...
for population in LWK JPT CHB TSI CEU YRI; do
	grep $population phase1_integrated_calls.20101123.ALL.panel >${INPUTDIR}/POPs_LIST/1000G.UnGenotyped.${population}.List
done
...
for Inv in ${INVERSIONS}; do
	...
	# VCF download (BP1END-2000000..BP2START+2000000)
	...
	cat $GENOFILE | tail -n+3 | cut -f3,1,$COL | awk -v OFS='\t' '{print $2,$1,$3;}' | sed "s/\tINV/\tII/" | sed "s/\tSTD/\tSS/" | sed "s/\tHET/\tSI/" >${INPUTDIR}/VCFs_and_BPs/${Inv}/HapMap.Genotyped.List
done
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
for Inv in In055 In045 In340 In069 In266; do
	#Get inversion chrm
